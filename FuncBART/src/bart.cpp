...
//--------------------------------------------------
void bart::draw(size_t funcP, size_t intvls, double sigma, rn& gen_fp, rn& gen_int, std::vector<double> r_vector)
{
  ...
  //double temp_fit{0.0};
   ...
   for(size_t j=0;j<m;j++) {
      ...
      bd(t[j],xi,di,pi,sigma,nv,pv,aug,gen_fp);
      drmu(t[j],xi,di,pi,sigma,gen_fp);
      fit(t[j],xi,p,n,x,ftemp);
