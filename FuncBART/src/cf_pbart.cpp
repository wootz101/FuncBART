...
RcppExport SEXP cf_pbart(
    ...
    SEXP _ixMatrix,		//x, train,  px(n*m) (transposed so rows are contiguous in memory)  | 2D Matrix

    SEXP _iy,		//y, train,  nx1
    SEXP _ixpMatrix,		//x, test, px(np*m) (transposed so rows are contiguous in memory)  | Need a 2D Matrix
...
)
{
  ...
  Rcpp::List x_Matrix(Rcpp::as<Rcpp::List>(_ixMatrix));
  ...
  printf("start xp \n");
  Rcpp::List xp_Matrix(Rcpp::as<Rcpp::List>(_ixpMatrix));
  std::vector<double*> ixp;
  for(int i=0; i <Rcpp::as<int>(_im); i++){
    printf("1 ");
    //Rcpp::NumericVector  xpv(_ixp);
    Rcpp::NumericVector xpv(Rcpp::as<Rcpp::NumericVector>(xp_Matrix[i]));
    printf("2 ");
    ixp[i] = &xpv[0];
    printf("Line 106 \n");
