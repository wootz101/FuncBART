...
/*
void makexinfoM(size_t p, size_t n, std::vector<double*> x_vector, std::vector<xinfo> xi_vector, size_t numcut, size_t m)
{

  for(int i; i< m; i++){
    double *_t_x = x_vector[i];
    xinfo& _t_xi = xi_vector[i];
    makexinfo(p, n,_t_x, _t_xi, numcut);
  }
}
 */
